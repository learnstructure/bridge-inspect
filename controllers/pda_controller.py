...
class PDAController:
    def __init__(self):
        ...
        self.original_results = None  # Cache for the first, unmodified results
    ...
    def upload_image(self, file_path):
        ...
        self.original_results = None  # Reset cache on new image
        if self.analysis_view: self.analysis_view.display_image(file_path)
        if self.mask_view: self.mask_view.display_image(file_path)
        if self.summary_view: self.summary_view.update_summary(None) # Clear summary
    ...
    def run_pda(self, file_path):
        ...
        # Cache the pristine results before any modifications
    ...
    def recalculate_results_with_new_mask(self, new_column_mask):
        ...
        # Always filter from the clean, original results
        ...
        # Generate a new overlay for the summary page based on the filtered results
        overlay = self._generate_final_overlay(self.current_file_path, filtered_results)
        filtered_results["overlay_image"] = overlay

        # Update only the mask and summary views with the temporary, filtered results
        if self.mask_view: self.mask_view.display_updated_results(filtered_results)
    ...
    def restore_original_results(self):
        """Restores the original, unmodified analysis results to all views."""
        if not self.original_results or not self.current_file_path:
            logger.warning("No original results available to restore.")
            return
        # Resend the original results to all pages, restoring the default state
    ...
    def _filter_defects_by_roi(self, original_results, new_column_mask):
        # Use deepcopy to avoid modifying the cached original_results
        ...
        for i, roi in enumerate(original_results["rois"]):
            ...
            if class_names.get(class_id) == 'column': continue
            if iou(roi, new_column_mask) > 0.9:
                filtered_rois.append(roi)
                filtered_class_ids.append(class_id)
                filtered_scores.append(original_results["scores"][i])
                if class_names.get(class_id) == 'spalling': spalling_area_in_mask += get_bbox_area(roi)
        ...
        recounted_results.update({
            ...
            "num_exposed_horizontal_bars": sum(1 for cid in filtered_class_ids if class_names.get(cid) == 'horizontal'),
            "num_exposed_vertical_bars": sum(1 for cid in filtered_class_ids if class_names.get(cid) == 'vertical'),
    ...
    def update_damage_assessment(self, updated_data):
        ...
        # This method is for when rules change. We need to decide if it updates the original or the modified view.
        # Current logic: It updates whatever is currently shown. This seems reasonable.
    ...
    def _update_all_views(self, file_path, results):
        # Use a deepcopy to ensure views get their own mutable copy
        ...
        if self.mask_view:
            if self.mask_view.current_file_path != file_path: self.mask_view.display_image(file_path)
            self.mask_view.display_updated_results(results_copy)
    ...
    def _create_detection_overlay(self, file_path, results, score_threshold=0.5):
        try:
            ...
            for i, roi in enumerate(results.get("rois", [])):
                ...
                class_name = class_names.get(results["class_ids"][i], 'unknown')
                y1, x1, y2, x2 = roi
